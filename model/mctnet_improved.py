# model/mctnet_improved.py
import torch
import torch.nn as nn
import numpy as np

# ============================================
# ECALayer (Efficient Channel Attention)
# ============================================
...
# ============================================
# CTFusion Original (pour référence)
# ============================================
...
# ============================================
# MCTNet Original (pour référence)
# ============================================
...
# ============================================
# NOUVEAU : Cross-Attention Fusion
# ============================================
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout=0.5):
        ...
        # Attention de Transformer vers CNN
        self.trans_to_cnn = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=True)
        self.norm_cnn = nn.LayerNorm(d_model)
        self.norm_trans = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, cnn_out, trans_out):
        # CNN interroge Transformer
        cnn_attended, _ = self.cnn_to_trans(cnn_out, trans_out, trans_out)
        cnn_out = self.norm_cnn(cnn_out + self.dropout(cnn_attended))
        
        # Transformer interroge CNN
        trans_attended, _ = self.trans_to_cnn(trans_out, cnn_out, cnn_out)
        trans_out = self.norm_trans(trans_out + self.dropout(trans_attended))
...
# ============================================
# CTFusion Amélioré (avec Cross-Attention)
# ============================================
class CTFusionImproved(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Fusion par Cross-Attention (au lieu de simple addition)
        out = self.cross_attn(cnn_out, trans_out)
        return out, attn_weights
...
class MCTNetImproved(nn.Module):
    ...
    def __init__(self, input_channels=13, time_steps=36, d_model=64, nhead=4,
        ...
        for i in range(n_stages):
            use_alpe = (i == 0)
            self.stages.append(CTFusionImproved(d_model, nhead, kernel_sizes, use_alpe, dropout))
...
if __name__ == "__main__":
    ...
    # Test forward pass
    x = torch.randn(4, 36, 13).to(device)
    mask = torch.ones(4, 36, 10).to(device)
